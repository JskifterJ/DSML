...
# Display each pollutant result with graph and bullet points
for pollutant_name, file_name, bullet_points in pollutant_graphs:
    st.markdown(f"### 📈 {pollutant_name}")
    
    st.image(f"../figures/analysis/{file_name}", use_column_width=True)
...
# --- New Subsection: Model Results per Pollutant ---
st.subheader("🔍 Model Results per Pollutant")

st.markdown(
    """
    Looking at overall model performance, we now dive deeper into the results of our regression models.  
    To ensure clarity, isolate the EV signal, and highlight model differences, we focus here on the **full-week, rush-hour** time scale.  
    For each pollutant, we show the regression output by country and model.
    """
)

# Define pollutant names and key findings
pollutant_info = {
    "CO₂": [
        "Linear, Ridge, and Lasso perform similarly with R² around 0.78–0.84 — good fit overall.",
        "CO₂ shows strong correlation with EV fleet share in several countries, especially for weekend peaks.",
        "Random Forest slightly underperforms simpler models here."
    ],
    "NO₂": [
        "Random Forest dramatically outperforms linear models (R² ≈ 0.88 vs. <0.5).",
        "Nonlinear relationships dominate for NO₂ — EV impact appears indirect or context-dependent.",
        "Weekend rush-hour variance captured better by tree-based model."
    ],
    "PM10": [
        "Moderate R² across all models (~0.45–0.69), indicating medium explainability.",
        "Linear models and Random Forest both capture signals in PM10 evolution.",
        "Some outliers persist in countries like NO, indicating potential measurement noise."
    ],
    "PM2.5": [
        "Random Forest achieves highest R² (~0.76), indicating a well-modeled pollutant.",
        "Lasso and Ridge still perform decently, suggesting strong linear trends in some countries.",
        "High fit suggests PM2.5 is responsive to EV penetration, especially during rush hours."
    ]
}

# Path to the analysis folder
analysis_folder = "../figures/analysis"

# Loop through pollutants and display corresponding graphs and insights
for pollutant, insights in pollutant_info.items():
    st.markdown(f"### 📈 {pollutant}")

    # Dynamically fetch the graph for the pollutant with 'AnnualAvg_fullweek_RushHour' in the filename
    graph_files = [
        f for f in os.listdir(analysis_folder)
        if pollutant in f and "AnnualAvg_fullweek_RushHour" in f
    ]

    if graph_files:
        # Display the first matching graph (assuming one graph per pollutant)
        graph_path = os.path.join(analysis_folder, graph_files[0])
        st.image(graph_path, caption=f"{pollutant} Regression Results", use_column_width=True)
    else:
        st.warning(f"No graph found for {pollutant} with 'AnnualAvg_fullweek_RushHour' in the filename.")
