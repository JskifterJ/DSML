...
# Conditional logic for other sections
    ...
    print("File exists:", os.path.exists("../results/best_model_per_pollutant_target.csv"))
    print("Absolute path:", os.path.abspath("../results/best_model_per_pollutant_target.csv"))
    ...
    *“All models are wrong, but some are useful.”* — George Box
    ...
    st.write(f"- **Best overall:** {best['Model']} for {best['Pollutant']} ({best['Target']}) with R² = {best['R2_test']:.2f}")
    ...
    def show_model_analysis(pollutant_name, bullet_points):
        image_path = f"../figures/analysis/{pollutant_name}_AnnualAvg_fullweek_RushHour_regression_country_fixed_effects_model_colored.png"
        if os.path.exists(image_path):
            st.image(Image.open(image_path), caption=f"Regression results for {pollutant_name}")
        else:
